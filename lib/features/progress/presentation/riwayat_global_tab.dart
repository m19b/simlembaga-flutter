...
class RiwayatGlobalTab extends StatefulWidget {
  ...
  @override
  State<RiwayatGlobalTab> createState() => RiwayatGlobalTabState();
}

class RiwayatGlobalTabState extends State<RiwayatGlobalTab>
    with SingleTickerProviderStateMixin, AutomaticKeepAliveClientMixin {
  final GlobalKey<_HariSubTabState> _hariKey = GlobalKey<_HariSubTabState>();

  void setTanggal(DateTime date) {
    _hariKey.currentState?.setTanggal(date);
  }
  @override
      ...
      children: [
        ...
        Expanded(
          child: TabBarView(
            controller: _tabController,
            physics:
                const NeverScrollableScrollPhysics(), // swipe can interfere with horizontal chart
            children: [
              _HariSubTab(key: _hariKey, repository: widget.repository),
...
class _HariSubTab extends StatefulWidget {
  final TahsinRepository repository;
  const _HariSubTab({super.key, required this.repository});

  @override
  State<_HariSubTab> createState() => _HariSubTabState();
}

class _HariSubTabState extends State<_HariSubTab>
            ...
            child: _isLoading
                ...
                ? Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          'Belum ada evaluasi untuk hari ini',
                          style: TextStyle(
                            color: Theme.of(
                              context,
                            ).colorScheme.onSurface.withValues(alpha: 0.6),
                ...
                : RefreshIndicator(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      final isLulus =
                          item['status_halaman']?.toString().toLowerCase() ==
                          'lulus';
                      String waktuText = '-';
                      if (item['created_at'] != null) {
                        try {
                          waktuText = DateFormat('HH:mm').format(
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Container(
                                        ...
                                        child: Text(
                                          '${item['tingkat'] ?? ''} ${item['nama_kelompok'] ?? ''}'
                                              .trim(),
                                          style: TextStyle(
                                            fontSize: 9,
                                            color: Theme.of(
                                ...
                                Container(
                                  ...
                                  child: Text(
                                    ...
                                    style: TextStyle(
                                      fontSize: 9,
                                      fontWeight: FontWeight.bold,
                                      color: isLulus ? kPrimary : kDanger,
                                    ),
                                  ),
                                ),
                                const SizedBox(height: 4),
...
class _MingguSubTabState extends State<_MingguSubTab>
    with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true;
  ...
  final _weekScrollCtrl = ScrollController(initialScrollOffset: 150.0);

  @override
  void initState() {
    super.initState();
    _initRanges();
    // Defer API call agar tidak blocking saat widget tree sedang dibangun
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      setState(() {
        _totalLulusChart =
            int.tryParse(data['total_lulus']?.toString() ?? '0') ?? 0;
        _totalUlangChart =
            int.tryParse(data['total_ulang']?.toString() ?? '0') ?? 0;
              ...
              children: [
                Expanded(
                  child: ListView.builder(
                    ...
                    controller: _weekScrollCtrl, // agak ke kanan
                    itemBuilder: (context, i) {
                      final r = _ranges[i];
                      final isSelected = i == _selectedIndex;
                      final txt =
                          '${DateFormat('d MMM').format(r.start)} - ${DateFormat('d MMM').format(r.end)}';
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'Minggu Ini: $_totalLulusChart Lulus / $_totalUlangChart Tidak Lulus',
                  style: TextStyle(
                    fontSize: 14,
                    color: Theme.of(
                      context,
                                ...
                                touchTooltipData: BarTouchTooltipData(
                                  ...
                                  getTooltipItem:
                                      (group, groupIndex, rod, rodIndex) {
                                        ...
                                        final lulus =
                                            data['lulus']?.toString() ?? '0';
                                        final ulang =
                                            data['ulang']?.toString() ?? '0';
